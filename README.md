...
Android 0.6.1 默认使用内置 Eclipse JGit 6.4 标准模式：通过 `ls-remote` 读取分支目录，首次对默认分支执行历史深度 500 的浅克隆，用户切换分支时再按需浅克隆该单一分支。临时仓库在分析完成或失败后都会删除；持久化内容只有图谱和聚合报表。远程 API 仍可在设置中按需启用，Web 版继续使用该 API。

GitHub 和 GitLab 使用 `blob:none` 过滤传输，只下载提交关系和目录树，不下载文件正文；热点统计复用同一对象读取器并只抽样最近 250 个提交，以降低导入时间、网络流量与手机端 CPU 占用。

提交图谱使用固定 1:1 分支切换斜率，Merge 节点以菱形和父提交数量标识，第一父提交保持为主线。图谱页支持按提交内容、作者、完整/短哈希、引用及远程分支名称搜索，并可从分支搜索结果直接加载对应单分支图谱。
